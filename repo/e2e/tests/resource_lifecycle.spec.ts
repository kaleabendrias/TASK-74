...
test('full resource lifecycle: create → review → publish (cross-role)', async ({ page }) => {
  ...
  // Wait for form submission to complete (API call + nav.push to resource list)
  await page.waitForSelector('#resource-search', { timeout: 15_000 });

  // Step 2: Verify the resource persists in the list
  ...
  // Step 3: Navigate to resource detail and submit for review
  await page.getByText(uniqueTitle).click();
  await page.waitForSelector('#btn-submit-review', { timeout: 15_000 });
  await page.click('#btn-submit-review');

  await expect(page.locator('#btn-submit-review')).toHaveCount(0, { timeout: 15_000 });

  // Step 4: Reviewer logs in and publishes
  await logout(page);
  ...
  await page.getByText(uniqueTitle).click();
  await page.waitForSelector('#btn-publish', { timeout: 15_000 });
  await page.click('#btn-publish');

  await expect(page.locator('#btn-publish')).toHaveCount(0, { timeout: 15_000 });

  // Step 5: Admin verifies published
  await logout(page);
